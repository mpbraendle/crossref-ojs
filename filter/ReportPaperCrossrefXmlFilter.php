...
/**
 * @file plugins/generic/crossref/filter/ReportPaperCrossrefXmlFilter.php
 *
 * Copyright (c) 2014-2025 Simon Fraser University
 * Copyright (c) 2000-2025 John Willinsky
 * Distributed under The MIT License. For full terms see the file LICENSE.
 *
 * @class ReportPaperCrossrefXmlFilter
 *
 * @ingroup plugins_generic_crossref
 *
 * @brief Class that converts a submission of type report-paper to a Crossref XML document.
 */
...
class ReportPaperCrossrefXmlFilter extends IssueCrossrefXmlFilter
{
    /**
     * Constructor
     *
    ...
    public function __construct($filterGroup)
    {
        ...
        $this->setDisplayName('Crossref XML report paper export');
    ...
    public function &process(&$pubObjects)
    {
        ...
        foreach ($pubObjects as $pubObject) {
            ...
            $reportPaperNode = $this->createReportPaperNode($doc, $pubObject);
            $bodyNode->appendChild($reportPaperNode);
        }
    ...
    /**
     * Create and return the node 'report-paper'
     *
     ...
     * @return \DOMElement
     */
    public function createReportPaperNode($doc, $submission)
    {
        ...
        $reportPaperNode = $doc->createElementNS($deployment->getNamespace(), 'report-paper');

        $reportPaperMetadataNode = $doc->createElementNS($deployment->getNamespace(), 'report-paper_metadata');
        $reportPaperMetadataNode->setAttribute('language', LocaleConversion::getIso1FromLocale($locale));
        ...
        if ($authors->count() != 0) {
            ...
            $reportPaperMetadataNode->appendChild($contributorsNode);
        }

        // titles
        // for report paper, only 1 titles element is allowed
        ...
        foreach ($titleLanguages as $lang) {
            ...
            $reportPaperMetadataNode->appendChild($titlesNode);
            $languageCounter++;
        ...
        // edition number (in OJS, there is only 1 edition)
        $editionNumberNode = $doc->createElementNS($deployment->getNamespace(), 'edition_number', '0');
        $reportPaperMetadataNode->appendChild($editionNumberNode);
        ...
        foreach($abstracts as $lang => $abstract) {
            ...
            $reportPaperMetadataNode->appendChild($abstractNode);
        }

        // publication date
        if ($datePublished = $publication->getData('datePublished')) {
            $publicationDateNode = $this->createDateNode($doc, $datePublished, 'publication_date');
            $publicationDateNode->setAttribute('media_type', 'online');
            $reportPaperMetadataNode->appendChild($publicationDateNode);
        }
        ...
        if ($editorDecision) {
            $reportPaperMetadataNode->appendChild($this->createDateNode($doc,  $editorDecision->getData('dateDecided'), 'approval_date'));
        }

        // isbn - not available

        // publisher
        // for missing data, do a lookup at ROR
        $publisher = $context->getData('publisherInstitution');
        $publisher_ror_data = $this->lookupROR($publisher);
        if ($publisher_ror_data) {
            if ($geonames = $publisher_ror_data->{'items'}[0]->{'locations'}[0]->{'geonames_details'}) {
                $city = $geonames->{'country_subdivision_name'};
                $country = $geonames->{'country_name'};
                $place = $city . ', ' . $country;
            }

            $ror_id = $publisher_ror_data->{'items'}[0]->{'id'};
        }
        
        $publisherNode = $doc->createElementNS($deployment->getNamespace(), 'publisher');
        $publisherNode->appendChild($doc->createElementNS($deployment->getNamespace(), 'publisher_name', 
            html_entity_decode($publisher, ENT_COMPAT, 'UTF-8')));
        if ($place) {
            $publisherNode->appendChild($doc->createElementNS($deployment->getNamespace(), 'publisher_place', 
                html_entity_decode($place, ENT_COMPAT, 'UTF-8')));
        }

        $reportPaperMetadataNode->appendChild($publisherNode);

        // institution 
        if ($ror_id) {
            $institutionNode = $doc->createElementNS($deployment->getNamespace(), 'institution');
            $institutionIdNode = $doc->createElementNS($deployment->getNamespace(), 'institution_id');
            $institutionIdNode->setAttribute('type', 'ror');
            $institutionIdNode->appendChild($doc->createTextNode($ror_id));
            $institutionNode->appendChild($institutionIdNode);
            $reportPaperMetadataNode->appendChild($institutionNode);
        }

        // publisher item - not available
        ...
        if ($publication->getData('licenseUrl')) {
            ...
            $reportPaperMetadataNode->appendChild($licenseNode);
        }
        ...
        $reportPaperMetadataNode->appendChild($doiDataNode);

        // component list (supplementary files)
        if (!empty($componentGalleys)) {
            $reportPaperMetadataNode->appendChild($this->createComponentListNode($doc, $submission, $componentGalleys));
        }

        // citation_list - ToDo

        $reportPaperNode->appendChild($reportPaperMetadataNode);
        return $reportPaperNode;
    }
    ...
    /**
     * Lookup an institution (e.g. the publisher) at the ROR API
     *
    ...
    public function lookupROR($institution)
    {
        $endpoint = 'https://api.ror.org/v2/organizations';
        $url = $endpoint . '?query=' . urlencode('"' . $institution . '"');
